...
class CategoricalDistRLTest(tf.test.TestCase):
  ...
  def assertEachInputRankAndCompatibilityChecked(
      self, nt, inputs, invalid_inputs, nt_name):
...
class CategoricalDistQLearningTest(CategoricalDistRLTest):
  ...
  def setUp(self):
    ...
    self.inputs = [
        ...
        self.atoms_t, self.logits_q_t
    ]

  def testRankCheck(self):
    alt_inputs = [tf.zeros((), tf.float32) for _ in self.inputs]
  ...
  def testCompatibilityCheck(self):
    alt_inputs = [tf.zeros([1], tf.float32) for _ in self.inputs]
    self.assertEachInputRankAndCompatibilityChecked(
        rl.categorical_dist_qlearning, self.inputs, alt_inputs,
        "CategoricalDistQLearning")
  ...
  def testTarget(self):
    ...
    expected = np.array([[0.0, 0.0, 1.0],
                         ...
                         [0.4, 0.6, 0.0],
                         [0.1, 0.5, 0.4]])
    outputs = rl.categorical_dist_qlearning(*self.inputs)
    self.assertAllClose(outputs.extra.target, expected, atol=1e-3)

  def testLoss(self):
    # Loss is CE between logits_q_tm1 [a_tm1] and target.
    outputs = rl.categorical_dist_qlearning(*self.inputs)
    expected = np.array([9.0, 3.69, 9.0, 0.69, 5.19])
    self.assertAllClose(outputs.loss, expected, atol=1e-2)
  ...
  def testGradQtm1(self):
    ...
    gradients = tape.gradient(neg_loss, [self.logits_q_tm1])

    grad_q_tm1 = gradients[0].numpy()
  ...
  def testNoOtherGradients(self):
    ...
    with tf.GradientTape() as tape:
      outputs = rl.categorical_dist_qlearning(*self.inputs)
    gradients = tape.gradient([outputs.loss],
...
class CategoricalDistDoubleQLearningTest(CategoricalDistRLTest):
  ...
  def testCompatibilityCheck(self):
    alt_inputs = [tf.zeros([1]) for _ in self.inputs]
    self.assertEachInputRankAndCompatibilityChecked(
        rl.categorical_dist_double_qlearning, self.inputs, alt_inputs,
        "CategoricalDistDoubleQLearning")
  ...
  def testTarget(self):
    ...
    expected = np.array([[0.0, 0.0, 1.0],
                         ...
                         [0.1, 0.5, 0.4]])
    self.assertAllClose(
        rl.categorical_dist_double_qlearning(*self.inputs).extra.target,
        expected, atol=1e-3)

  def testLoss(self):
    expected = np.array([9.0, 3.69, 9.0, 0.69, 5.19])
    self.assertAllClose(
        rl.categorical_dist_double_qlearning(*self.inputs).loss,
        expected, atol=1e-2)
  ...
  def testGradQtm1(self):
    ...
    with tf.GradientTape() as tape:
      tape.watch(self.logits_q_tm1)
      outputs = rl.categorical_dist_double_qlearning(*self.inputs)
      neg_loss = -outputs.loss
    # Take gradients of the negative loss, so that the tests here check the
    # values propagated during gradient _descent_, rather than _ascent_.
    gradients = tape.gradient(neg_loss, [self.logits_q_tm1])
...
class CategoricalDistTDLearningTest(CategoricalDistRLTest):
  ...
  def testTarget(self):
    ...
    expected = np.array([[0.0, 0.0, 1.0],
                         ...
                         [0.4, 0.6, 0.0],
                         [0.1, 0.5, 0.4]])
    outputs = rl.categorical_dist_td_learning(*self.inputs)
    self.assertAllClose(
        outputs.extra.target, expected, atol=1e-3)
  ...
  def testGradVtm1(self):
    ...
    # values propagated during gradient _descent_, rather than _ascent_.
    with tf.GradientTape() as tape:
      tape.watch(self.logits_v_tm1)
      outputs = rl.categorical_dist_td_learning(*self.inputs)
      neg_loss = -outputs.loss
    gradients = tape.gradient(neg_loss, [self.logits_v_tm1])
    grad_v_tm1 = gradients[0].numpy()
  ...
  def testNoOtherGradients(self):
    ...
    gradients = tape.gradient(
        outputs.loss,
        [self.logits_v_t, self.r_t, self.pcont_t, self.atoms_t, self.atoms_tm1])
    self.assertEqual(gradients, [None for _ in gradients])
